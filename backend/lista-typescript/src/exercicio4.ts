...
const listaDeFuncionarios: Array<Funcionarios> = [
...
const filtraFuncionario:(listaDeFuncionarios: Funcionarios[]) => Funcionarios[] = 
    (listaDeFuncionarios) => {
    return listaDeFuncionarios.filter((funcionario) => {
        return funcionario.setor === SETORES.MARKETING && funcionario.presencial === true
    })
}
console.log(filtraFuncionario(listaDeFuncionarios));
